...
export class Studio {
  ...
  private mount() {
    ...
    bind('copy-title',()=>void this.run('复制标题…',async()=>{const title=this.draft.title.trim();if(!title)throw new Error('请先输入标题或选择笔记。');await this.host.copy(title);this.tell('标题已复制，请粘贴到平台的标题栏。');}));
  ...
  private render(){
    ...
    this.q('[data-action="copy"]').textContent=platform==='x'&&this.previewMode==='thread'?'复制整组串文':'复制正文';
    ...
    this.q('.mg-counts').textContent=`${count.toLocaleString()} 字符`+(platform==='x'?` · ${splitThread(this.bodyContent().plainText).length} 条串文`:'');
  ...
  private bodyContent(){return renderDraft(this.draft,{...this.options(),includeTitle:false},this.assets);}
  private async copy(){
    const textOnly=this.settings.platform==='xiaohongshu'||this.settings.platform==='x'&&this.previewMode==='thread';
    if(textOnly)this.fresh();else await this.completeContent();
    const content=this.bodyContent();
    if(!this.draft.markdown.trim())throw new Error('正文为空，请先输入正文。');
    ...
    this.tell(textOnly?'正文已复制（不含标题）；图片请通过内容包上传。':count?`正文及 ${count} 张图片已复制（不含标题）。粘贴后请等平台完成图片处理。`:'正文已复制（不含标题），请粘贴到平台正文编辑器。');
  }
  private async export(){
    ...
    let cardCount=0;
    if(platform==='x')splitThread(this.bodyContent().plainText).forEach((text,i)=>files.push({name:`thread-${String(i+1).padStart(2,'0')}.txt`,content:text}));
