...
export interface RenderOptions { platform: Platform; template: Template; fontSize: number; accent: string; footnotes: boolean; includeTitle?: boolean }
...
export const PLATFORMS: Record<Platform, { name: string; label: string; hint: string }> = {
  wechat: { name: '公众号', label: 'WECHAT', hint: '标题、正文分别复制到公众号对应输入区。' },
  zhihu: { name: '知乎', label: 'ZHIHU', hint: '标题与富文本正文分别复制，也可保存 Markdown。' },
