...
## 2026-09-09 后续三个平台失败核对

用户再次测试后，平台 DOM 与错误日志确认：小红书第一张仍为 `blob:`，`data-percent=0`，三个图片标记均保留；知乎正文只剩初始化字符，React 的 `removeChild` 异常栈指向墨稿 `DraftDriver.write`；X 正文可见但没有正文图片，自动保存阶段反复报 Draft.js 实体查找错误。不能把这些状态称为已上传或已保存成功。

Draft.js 修复使用页面自身 ContentState 构造器生成离屏字符元数据，去掉 `execCommand` 种子输入；无链接字符不再显式传 `entity: undefined`，保留库的 `null` 默认值。离线测试现在执行真正的 Draft.js 序列化与回读，防止仅检查显示 DOM 就漏掉保存错误。

小红书公开编辑器代码中的工具栏先调用异步尺寸读取/插图，然后立即发起上传；上传成功时只查找当时已存在的节点。这个先后顺序存在回调早于插图的竞态，与临时预览停滞相符，但没有网络回读证据证明用户本次失败仅由这个竞态引起。0.3.2 对 PNG / JPEG 调用其配置的原生图片回调，该路径先拿到上传地址再插图；只调用一次，不派发可能触发其他插件的 DOM 粘贴事件，也不访问系统剪贴板。其他格式仍使用原工具栏，未扩大保证范围。

这些改动仍待新版扩展在真实平台多图上传、自动保存与重开验收。未借助其他浏览器表面或接口绕过工具对扩展内部页面/管理页的拒绝。
